...
A random contributor ID is saved in `~/.call-susan-calvin/contributor-id` on your first donation and reused to count repeat contributions once. It contains no name or email and grants no access to donations. A different computer or deleting this file creates a new identity. The service publishes only aggregate contributor, session, and shared-text token counts.
