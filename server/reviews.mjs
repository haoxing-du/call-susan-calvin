...
export class Reviews {
  ...
  async donation(job, indices, consent, version, batchIndex) {
    ...
    return { ...(job.contributorId ? { contributorId: job.contributorId } : {}), ...(job.feedbackSnapshot ? { classifierFeedback: job.feedbackSnapshot.value } : {}), donationRunId: runId, collector: { version }, group: { id: job.id, index: batchIndex, count: job.batches.length }, redactionMode: job.options.mode, createdAt: consent.consentedAt, consent, redactionSummary: { automatedDetections: detections }, sessions };
  }
  async close() {
